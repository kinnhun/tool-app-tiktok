...
# Xác định thư mục gốc để tìm credentials và config (Tương thích hoàn hảo với PyInstaller)
...
def check_connection(url):
    ...
    try:
        ...
        # Return both title and id (gid) for each worksheet
        result['tabs'] = [{'title': ws.title, 'gid': str(ws.id)} for ws in spreadsheet.worksheets()]
        result['success'] = True
...
def get_tab_data(spreadsheet_id, tab_name):
    ...
    try:
        ...
        if not all_values:
            default_headers = ['STT', 'Link TikTok', 'Trạng thái', 'Tên sản phẩm', 'Giá hiện tại', 'Giá gốc', 'Giá sale', 'Link sản phẩm', 'Tên shop', 'Link Shop', 'Cập nhật lúc', 'Ghi chú']
            worksheet.update('A1:L1', [default_headers])
...
def update_row(spreadsheet_id, tab_name, row_number, data, column_mapping):
    ...
    try:
        ...
        # Shop link column
        if 'shop_link' in data and 'shop_link_col' in column_mapping:
            col = column_mapping['shop_link_col']
            cell = f'{col}{row_number}'
            updates.append({'range': cell, 'values': [[data['shop_link']]]})
...
def append_link(spreadsheet_id, tab_name, link, link_col, status_col, status="Chưa xử lý"):
    ...
    try:
        ...
        if not all_values:
            ...
            headers = ['STT', 'Link TikTok', 'Trạng thái', 'Tên sản phẩm', 'Giá hiện tại', 'Giá gốc', 'Giá sale', 'Link sản phẩm', 'Tên shop', 'Link Shop', 'Cập nhật lúc', 'Ghi chú']
...
def append_links_batch(spreadsheet_id, tab_name, links, link_col, status_col, status="Chưa xử lý"):
    """Append multiple links to the sheet at once to avoid API rate limits."""
    if not links: return True, [], 0
    try:
        client = get_client()
        spreadsheet = client.open_by_key(spreadsheet_id)
        worksheet = spreadsheet.worksheet(tab_name)
        ...
        rows_to_append = []
        stt_idx = headers.index('STT') if 'STT' in headers else -1
        
        start_stt = len(all_values)
        row_indices = []
        ...
        for i, link in enumerate(links):
            ...
            if stt_idx >= 0:
                if stt_idx < len(new_row):
                    new_row[stt_idx] = start_stt + i
                else:
                    new_row.extend([""] * (stt_idx - len(new_row) + 1))
                    new_row[stt_idx] = start_stt + i
                    
            rows_to_append.append(new_row)
            row_indices.append(start_stt + i + 1) # 1-indexed row number in Google Sheets
            
        worksheet.append_rows(rows_to_append, value_input_option='USER_ENTERED')
        return True, row_indices, len(rows_to_append)
    except Exception as e:
        print(f"Error appending batch links: {e}")
        return False, [], 0
